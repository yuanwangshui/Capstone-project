package com.example.home_safer.util.mqtt_tool;
...
public class aliyun_connect {
    ...
    public static int Temperature=0;//室内温度
    public static int humidity=0;//室内湿度
    public static int peculiar_smell=0;//0：无异味，1：天然气泄漏
    public static int fire=0;//0:无火，1：有火
    public static int gas=0;//0:正常，1:泄漏
    ...
    private static void Judge(String[] str) {
        for(int i=0;i<str.length-1;i++)
        {
            ...
            if(str[i].equals("gas"))//和异味匹配
            {
                gas = str[i + 1].charAt(1) - '0';//转换为int
//                if(str[i+1].charAt(3)==','| str[i+1].charAt(3)=='}')
//                {
...
//                }
            ...
            if(str[i].equals("flame"))//和火焰报警匹配
            {
                fire=str[i+1].charAt(1)-'0';

//                if(str[i+1].charAt(3)==','| str[i+1].charAt(3)=='}')
//                {
//                    fire=str[i+1].charAt(2)-'0';//转换为int
//                }
//                else
//                {
//                    fire=(str[i+1].charAt(2)-'0')*10+str[i+1].charAt(3)-'0';//转换为int，两个数字
//                }
            }
            if(str[i].equals("Temperature"))//和室内温度匹配
            {
                if(str[i+1].charAt(str[i+1].length()-2)=='}')
                    Temperature= Integer.parseInt(str[i+1].substring(1,str[i+1].length()-2));
                else
                    Temperature= Integer.parseInt(str[i+1].substring(1,str[i+1].length()-1));
            }
            if(str[i].equals("Humidity"))//和室内湿度匹配
            {
                //Humidity=Float.parseFloat(str[i+1].substring(2,str[i+1].length()-3));
                if(str[i+1].charAt(str[i+1].length()-2)=='}')
                    Humidity= Integer.parseInt(str[i+1].substring(1,str[i+1].length()-2));
                else
                    Humidity= Integer.parseInt(str[i+1].substring(1,str[i+1].length()-1));
                //break;
            }
