...
def get_128_features(person):  # person代表第几个人脸数据文件夹
    ...
    list_imgs = os.listdir(imgs_path)#获取图片名
    imgs_num = len(list_imgs)

    if os.path.exists(config.points_faceData_path + imgs_folder):
        shutil.rmtree(points_faceImage_path)#递归删除文件
    os.makedirs(points_faceImage_path)#创建文件
    print("人脸点图文件夹建立成功!!")

    with tqdm(total=imgs_num) as pbar:  # 导入进度条模块
        pbar.set_description(str(imgs_folder))# 设置进度条的描述为图片文件夹的名称
        for j in range(imgs_num):# 遍历图片文件夹中的所有图片
            image = cv.imread(os.path.join(imgs_path, list_imgs[j]))# 读取图片文件

            faces = config.detector(image, 1)  # 经查阅资料，这里的1代表采样次数   # 使用config.detector检测图片中的人脸，返回一个列表
            if len(faces) != 0:
    ...
    res = np.median(np_f, axis=0)#取数组的中位数
