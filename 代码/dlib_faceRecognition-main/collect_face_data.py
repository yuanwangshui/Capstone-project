...
class face_detect():
    ...
    def get_fps(self):  # 得到当前处理的帧率
        now = time.time()  # 用当前时间减去开始处理的时间
    ...
    def show(self, camera):  # 打开摄像头
        while camera.isOpened() and not self.quit_flag:  # 只有当摄像头打开了且退出程序的标志不为1则进入循环
            val, self.image = camera.read()
            if val == False: continue  # 没读取成功

            key = cv.waitKey(1)  # 1毫秒后自动关闭

            res = self.face_detecting() # 调用人脸检测方法，返回结果
            if res is not None:
                ...
                for i in range(self.face_num):
                    ...
                    if self.collect_face_data == True:
                        cv.putText(self.image, "Face", (int((left + right) / 2) - 50, bottom + 20),
